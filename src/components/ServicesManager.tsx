...
import { Service, Product, ServiceCategory } from '../types';
import { 
  getServices, 
  getProducts, 
  saveService, 
...
const ServicesManager: React.FC = () => {
  ...
  const [statistics, setStatistics] = useState(getServicesStatistics());
  ...
  const loadData = () => {
    ...
    setStatistics(getServicesStatistics());
  };
    ...
    try {
      if (serviceData.id) {
        ...
        if (existingService) {
          ...
          saveService(updatedService, currentUser.username);
        }
      } else {
        // Create new service
        createService(serviceData as Omit<Service, 'id' | 'createdAt' | 'updatedAt'>, currentUser.username);
      }
    ...
    try {
      if (productData.id) {
        ...
        if (existingProduct) {
          ...
          saveProduct(updatedProduct, currentUser.username);
        }
      } else {
        // Create new product
        createProduct(productData as Omit<Product, 'id' | 'createdAt' | 'updatedAt'>, currentUser.username);
      }
  ...
  const handleDeleteService = (serviceId: string) => {
    ...
    if (confirm('¿Estás seguro de que deseas eliminar este servicio?')) {
      deleteService(serviceId, currentUser.username);
      loadData();
    }
  };
  ...
  const handleDeleteProduct = (productId: string) => {
    ...
    if (confirm('¿Estás seguro de que deseas eliminar este producto?')) {
      deleteProduct(productId, currentUser.username);
      loadData();
    }
  };
  ...
  const handleBulkPriceUpdate = () => {
    ...
    const updates = selectedItems.map(serviceId => {
    ...
    if (confirm(`¿Actualizar precios de ${updates.length} servicios?`)) {
      bulkUpdatePrices(updates, currentUser.username);
      loadData();
      setSelectedItems([]);
          ...
          <div className="flex-1">
            <div className="flex items-center space-x-3">
              ...
              <span 
                ...
                style={{
                  backgroundColor: service.isActive !== false ? `${colors?.success || '#10b981'}1a` : `${colors?.error || '#ef4444'}1a`,
        ...
        <div className="flex-1">
          <div className="flex items-center space-x-3">
            ...
            <span 
              className="px-2 py-1 text-xs rounded-full theme-transition"
              style={{
                backgroundColor: product.isActive !== false ? `${colors?.success || '#10b981'}1a` : `${colors?.error || '#ef4444'}1a`,
...
const PriceHistoryModal: React.FC<PriceHistoryModalProps> = ({ serviceId, onClose }) => {
  const [priceHistory, setPriceHistory] = useState(getServicePriceHistory(serviceId));
              ...
              {priceHistory.map(history => (
                ...
                >
                  ...
                  <div 
                    ...
                    style={{
                      backgroundColor: history.newPrice > history.oldPrice ? `${colors?.error || '#ef4444'}1a` : `${colors?.success || '#10b981'}1a`,
                      color: history.newPrice > history.oldPrice ? (colors?.error || '#ef4444') : (colors?.success || '#10b981')
                    }}
                  >
